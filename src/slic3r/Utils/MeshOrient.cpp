...
std::vector<TriInfo> collect_triangles(const Model &model)
{
    ...
    for (const ModelObject *mo : model.objects) {
        if (!mo) continue;
        std::vector<TriInfo> per = collect_triangles_object(*mo);
        tris.insert(tris.end(), per.begin(), per.end());
    }
    return tris;
}

// One planar-face cluster: every triangle whose normal quantized to the same
// 0.001-precision (≈ 0.06° angular) lattice point. The reported `normal` is
// the area-weighted mean of the cluster's triangle normals (so it's smoother
// than any one triangle's raw normal).
struct FaceCluster {
    Vec3d  normal;
    double area_mm2;
    int    n_triangles;
...
// Minimal JSON helpers. The output is small + flat enough that hand-formatting
// beats pulling in a JSON library (the calib sidecar emitters use the same
// approach). All floats: %.6g (enough precision for mesh coords, no trailing
// noise); strings: backslash-escape quotes + backslashes only — mesh-local
// numbers and file paths don't carry control chars.
                     ...
                     std::ostream &out, int top_n_clusters)
{
    ...
    out << "  " << json_str("note") << ": " << json_str(
        "Coordinates are mesh-local — post-OrcaSlicer load-time centering. "
        "Use these directly with --ground-face-point and --ground-face-normal.") << ",\n";
