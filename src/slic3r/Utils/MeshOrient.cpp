...
// Iterate every triangle of every volume of one object, return their unit
// normals + areas (mesh-local). Skips degenerate (zero-area) triangles.
...
// area-weighted mean of the cluster's triangle normals.
struct FaceCluster {
    Vec3d  normal;
    double area_mm2;
    int    n_triangles;
...
// Cluster triangles by quantized normal direction; sorted by area desc.
...
// Minimal JSON helpers — hand-formatted (same approach as CalibCLI sidecar
// emitters). Floats: %.6g. Strings: escape " and \ only.
