...
int CLI::run(int argc, char **argv)
{
    ...
    //      compatible_printers_condition can be evaluated in the upwards check below. Previously only
    //      the literal compatible_printers list was extracted, which broke any condition-based system
    //      profile (most Prusa/non-BBL system process JSONs declare compatibility via condition only).
    ...
    std::string pipe_name, makerlab_name, makerlab_version, different_process_setting, different_machine_setting;
    ...
    //ORCA: resolve the `inherits` chain of presets loaded via --load-settings / --load-filaments.
    //      The CLI historically loaded each JSON in isolation; values that live in a parent preset
    //      (e.g. sparse_infill_density on fdm_process_common, inherited by Prusa's 0.20mm SPEED process)
    //      were missing from the merged print config and the slice silently used CLI defaults instead.
    //      Reuse the existing PresetBundle loader (same code path the GUI uses) to get full resolution
    //      including the renamed-parent fallback in find_preset2. The bundle is built lazily on first
    //      use and reused across all subsequent --load-* file loads in the same CLI invocation.
    std::unique_ptr<PresetBundle> cli_inherits_bundle;
    bool                          cli_inherits_bundle_failed = false;
    auto ensure_inherits_bundle = [&cli_inherits_bundle, &cli_inherits_bundle_failed]() -> PresetBundle* {
        if (cli_inherits_bundle)        return cli_inherits_bundle.get();
        if (cli_inherits_bundle_failed) return nullptr;
        try {
            auto bundle = std::make_unique<PresetBundle>();
            //ORCA: use the public umbrella load_presets() rather than the private
            //      load_system_presets_from_json + load_user_presets + update_system_maps trio.
            //      An empty AppConfig steers load_presets to its defaults: DEFAULT_USER_FOLDER_NAME for
            //      user presets, no preset_folder override, no preferred selections. The side methods
            //      it also calls (update_multi_material_filament_presets, update_compatible,
            //      load_selections, set_calibrate_printer) are harmless in CLI context.
            AppConfig empty_config;
            bundle->load_presets(empty_config, ForwardCompatibilitySubstitutionRule::EnableSilent);
            cli_inherits_bundle = std::move(bundle);
            BOOST_LOG_TRIVIAL(info) << "CLI: loaded preset bundle for inherits resolution ("
                << cli_inherits_bundle->prints.get_presets().size() << " process, "
                << cli_inherits_bundle->filaments.get_presets().size() << " filament, "
                << cli_inherits_bundle->printers.get_presets().size() << " machine)";
        } catch (const std::exception &ex) {
            BOOST_LOG_TRIVIAL(warning) << "CLI: failed to load preset bundle for inherits resolution: " << ex.what()
                << "; loaded JSONs will be used as-is (inherits chain unresolved)";
            cli_inherits_bundle_failed = true;
            return nullptr;
        }
        return cli_inherits_bundle.get();
    };
    auto resolve_inherits = [&ensure_inherits_bundle](DynamicPrintConfig &config, const std::string &config_type) {
        //ORCA: capture the inherits name as a std::string before mutating `config` below.
        //      The ConfigOptionString pointer would dangle after `config = std::move(merged)`.
        std::string inherits_name;
        if (const auto *inh = dynamic_cast<const ConfigOptionString *>(config.option(BBL_JSON_KEY_INHERITS)))
            inherits_name = inh->value;
        if (inherits_name.empty())
            return;
        PresetBundle *bundle = ensure_inherits_bundle();
        if (!bundle)
            return;
        const PresetCollection *coll = nullptr;
        if      (config_type == "process")  coll = &bundle->prints;
        else if (config_type == "filament") coll = &bundle->filaments;
        else if (config_type == "machine")  coll = &bundle->printers;
        if (!coll)
            return;
        const Preset *parent = coll->find_preset2(inherits_name, true);
        if (!parent) {
            BOOST_LOG_TRIVIAL(warning) << boost::format("CLI: parent preset '%1%' not found in bundle for inherits resolution; using child config as-is") % inherits_name;
            return;
        }
        DynamicPrintConfig merged = parent->config;
        merged.apply(config);
        //ORCA: record the user's override keys (child differs from parent) into different_settings_to_system
        //      so the 3MF export embeds them. Without this, GUI re-loading the 3MF reverts the inherited
        //      filament/process back to system defaults and shows the "unsaved changes" dialog. Mirrors
        //      what GUI Preset::save writes via config.diff(*parent_config). compatible_printers /
        //      compatible_prints are handled by separate tracking columns (inherits_group / dedicated
        //      compatible_printers slot) and must be excluded here.
        std::vector<std::string> override_keys = merged.diff(parent->config);
        //ORCA: inherits / *_settings_id are registered ConfigOptions (so they appear in diff()) but
        //      are preset metadata, not user-tunable settings. compatible_printers/compatible_prints
        //      have their own tracking column (inherits_group / per-slot list) and would double-count.
        override_keys.erase(std::remove_if(override_keys.begin(), override_keys.end(),
            [](const std::string &k) {
                return k == "compatible_printers" || k == "compatible_prints"
                    || k == "inherits" || k == "filament_settings_id"
                    || k == "print_settings_id" || k == "printer_settings_id";
            }),
            override_keys.end());
        auto *diff_opt = merged.option<ConfigOptionStrings>("different_settings_to_system", true);
        diff_opt->values.clear();
        diff_opt->values.push_back(Slic3r::escape_strings_cstyle(override_keys));
        config = std::move(merged);
        BOOST_LOG_TRIVIAL(info) << boost::format("CLI: resolved inherits via parent '%1%' (final config has %2% keys, %3% user overrides)") % inherits_name % config.keys().size() % override_keys.size();
    };
    ...
    auto load_config_file = [config_substitution_rule, &resolve_inherits](const std::string& file, DynamicPrintConfig& config, std::string& config_type,
        ...
        try {
            ...
            //ORCA: if the loaded JSON declares an `inherits` parent, walk the chain via the lazily-built
            //      PresetBundle and merge parent config + child overrides. Done before normalize_fdm so
            //      normalization sees the complete merged config.
            resolve_inherits(config, config_type);
    ...
    for (auto const &file : load_configs) {
        ...
        if (config_type == "machine") {
            ...
            //printer_inherits = config.option<ConfigOptionString>("inherits", true)->value;
            //ORCA: capture user override keys recorded by resolve_inherits into different_machine_setting,
            //      to be applied to different_settings[filament_count+1] below (mirrors the process path
            //      a few blocks above). Erase from config so update_full_config doesn't later copy the
            //      resolver's single-slot marker over m_print_config's per-slot
            //      different_settings_to_system array.
            if (!is_bbl_3mf && config.option<ConfigOptionStrings>("different_settings_to_system")) {
                ...
                if (!diff_settings.empty())
                    different_machine_setting = diff_settings[0];
            }
            ...
            //ORCA: drop the resolver-stashed marker so update_full_config doesn't copy this single-slot
            //      value over m_print_config's per-slot different_settings_to_system array.
            config.erase("different_settings_to_system");
    ...
    //ORCA: Replace the four manual equality-loop checks below with is_compatible_with_printer(), which
    //      also evaluates compatible_printers_condition. Non-BBL system process profiles (e.g. Prusa,
    //      Voron) typically leave compatible_printers empty and declare compatibility via condition only,
    //      so the previous literal-name match always reported them as incompatible. Falls back to the
    //      old literal behaviour automatically: when both list and condition are empty,
    //      is_compatible_with_printer returns true (matches the "old 3mf, no compatible printers" path).
    ...
    if (!new_printer_name.empty()) {
        if (!new_process_name.empty()) {
            //new process + new printer: both configs loaded from --load-settings
            ...
            //old process (3MF-embedded) vs new printer: fall back to literal match if current process config not preserved
        ...
        //new process vs current printer (3MF-embedded)
        ...
        //both current (pure 3MF reprocess)
    ...
    if (!new_printer_name.empty() || up_config_to_date) {
        ...
        if (new_printer_name.empty()) {
            ...
            //ORCA: carry over user override keys from the loaded machine JSON so 3MF export marks them dirty.
            different_settings[filament_count+1] = different_machine_setting;
    ...
    if (!new_process_name.empty() || up_config_to_date) {
        ...
        if (new_process_name.empty()) {
            ...
            //ORCA: carry over user override keys from the loaded process JSON so 3MF export marks them
            //      dirty. different_process_setting was captured at the load site above from the merged
            //      config's different_settings_to_system slot 0.
            different_settings[0] = different_process_setting;
    ...
    if ((load_filament_count > 0) || (up_config_to_date))
    {
        ...
        for (int index = 0; index < load_filaments_config.size(); index++) {
            ...
            if (load_filament_count > 0) {
                ...
                //ORCA: carry over user override keys from the loaded filament JSON so 3MF export marks
                //      them dirty. resolve_inherits stashed the override list in the config's
                //      different_settings_to_system[0]; after reading, erase from config so the opt_key
                //      loop below doesn't blindly copy the single-slot resolver marker over
                //      m_print_config's per-slot different_settings_to_system.
                different_settings[filament_index] = "";
                if (auto *fil_diff = config.option<ConfigOptionStrings>("different_settings_to_system"))
                    if (!fil_diff->values.empty())
                        different_settings[filament_index] = fil_diff->values[0];
                config.erase("different_settings_to_system");
    ...
    bool no_check = false;
    bool strict_mode = false;   //ORCA: --strict — see PrintConfig.cpp comment
    ...
    for (auto const &opt_key : m_actions) {
        if (opt_key == "help") {
            ...
            //      that matches 0 facets makes the action fail.
            const std::string source = m_input_files.empty() ? std::string("<no input>") : m_input_files.front();
            const std::string spec   = m_config.opt_string(opt_key);
            bool all_ok = true;
            for (Model &model : m_models) {
                model.add_default_instances();
